...
@section('content')
<div class="heading text-center font-bold text-2xl m-5 text-gray-800">تــــعـــديل منــــــشـــــور</div>
    @if ($posts->count())
    @foreach($posts as $post)

    <form action="{{route('updatepost', $post) }}" method="post"  enctype="multipart/form-data" class="editor bg-white mx-auto w-8/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
        @csrf
    ...
    <div class="buttons flex mt-4">
        <button type="submit" class="btn border border-indigo-500 p-1 px-4 font-semibold cursor-pointer text-gray-200 ml-2 bg-indigo-500">
            <span class="inline-block mr-2">حفظ</span>
